...
The page contains exactly three visible content sections and no sidebar, title banner,
summary cards, snapshot selectors, filters, freshness text, warnings, or technical
metadata. It automatically uses the most recently created snapshot and keeps all
same-season snapshots only for the evolution chart.
